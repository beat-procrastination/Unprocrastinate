...
function findLastRepeat(startDate, interval, intervalType, beforeDate = null) {
    const [startDay, startMonth, startYear] = startDate.split('.').map(Number);
    const currentDate = new Date();
    const before = beforeDate
        ? new Date(beforeDate.split('.').reverse().join('-'))
        : currentDate;

    if (isNaN(before)) {
        throw new Error("Invalid beforeDate.");
    }
    ...
    switch (intervalType.toLowerCase()) {
        case 'day':
            ...
            const timeDiff = before - start;

            if (timeDiff < 0) {
                throw new Error("The 'beforeDate' is earlier than the 'startDate'.");
            }

            const daysDiff = Math.floor(timeDiff / (1000 * 60 * 60 * 24));
            const lastDayCount = Math.floor(daysDiff / interval);
            lastRepeat = new Date(start.getTime() + lastDayCount * interval * 24 * 60 * 60 * 1000);
            break;

        case 'month':
            lastRepeat = new Date(startYear, startMonth - 1, startDay);
            while (lastRepeat <= before) {
                const nextRepeat = new Date(lastRepeat);
                nextRepeat.setMonth(nextRepeat.getMonth() + interval);
                if (nextRepeat >= before) break;
                lastRepeat = nextRepeat;
            }
            break;
        ...
        case 'year':
            ...
            while (lastRepeat <= before) {
                const nextRepeat = new Date(lastRepeat);
                nextRepeat.setFullYear(nextRepeat.getFullYear() + interval);
                if (nextRepeat >= before) break;
                lastRepeat = nextRepeat;
            }
            break;
        ...
        default:
            throw new Error("Invalid intervalType. Use 'day', 'month', or 'year'.");
    }

    // Format as "YYYY-MM-DD"
    return lastRepeat.toISOString().split('T')[0];
}

// Example usage
console.log(findLastRepeat("12.09.2024", 4, "day")); // Default `beforeDate` is today
console.log(findLastRepeat("12.09.2024", 2, "month", "10.04.2025"));
console.log(findLastRepeat("12.09.2024", 1, "year", "10.04.2026"));
